...
trait EnumConcern
{
    ...
    public static function toOption(array $excludes = []): array
    {
        ...
        return collect(self::cases())
            ->filter(fn($item): bool => !\in_array($item->value, $excludes, true))
    ...
    public static function labels(): array
    {
        ...
        if (!\in_array('App\Core\Enum\Contracts\HasLabel', class_implements(static::class), true)) {
            return [];
        }

        foreach (self::all() as $enumKey => $enumValue) {
            $labels[$enumKey] = static::class::{$enumKey}->getLabel();
        }
